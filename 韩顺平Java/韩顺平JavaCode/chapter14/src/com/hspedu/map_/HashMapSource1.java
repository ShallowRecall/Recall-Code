    ...
    public static void main(String[] args) {
        ...
        final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
                ...
                if ((p = tab[i = (n - 1) & hash]) == null)
                    ...
                    Node<K,V> e; K k;//辅助变量
                    if (p.hash == hash &&
                // 如果table的索引位置的key 的hash相同和新的key的hash值相同，
                ...
                //就认为不能加入新的k
                        ...
                        for (int binCount = 0; ; ++binCount) {//死循环
                            if ((e = p.next) == null) {//如果整个链表，没有和他相同的，就加到该链表的最后
                                ...
                                //就调用 treeifBin 方法进行红黑树的转换
                                if (binCount >= TREEIFY_THRESHOLD - 1) // -1 for 1st
                                    treeifyBin(tab, hash);
                                break;
                            }
                            if (e.hash == hash &&   //如果在循环比较过程中，发现有相同的，就break，就只是替换value
                ...
                5.关于树化(转成红黑树)
                //如果table 为null，或者大小还没有到 64，就暂时不树化，而是进行扩容。
                //否则才会真正的树化 -> 剪枝
