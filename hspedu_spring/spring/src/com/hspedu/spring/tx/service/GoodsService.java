...
public class GoodsService {
     ...
     * 5. 在执行buyGoodsByTX()方法 先调用事务管理器的 doBegin() , 再调用 buyGoodsByTX()
     * 如果执行中没有发生异常，则调用 事务管理器的 doCommit(), 如果发生异常 调用事务管理器的 doRollback()
     */
    @Transactional
     ...
     * 2. 我们将buyGoodsByTxISOLATION()方法的隔离级别设置为 Isolation.READ_COMMITTED
     * ，表示只要是提交的数据，在当前事务是可以读取到最新数据的
     */
     ...
     * ，该事务就进行回滚.
     * 3. 如果没有设置 timeout，默认是 -1，表示使用事务的默认超时时间，
     * 或者不支持
     */
