 ...
 *
 */
@Order(value = 2)// 表示该切面类执行的顺序，value的值越小，优先级越高，越先执行
...
public class SmartAnimalAspect {
    ...
    /**
     * 解读：
     * 1. @Before 表示前置通知，即在我们的目标对象方法执行前执行
     ...
     * 3. showBeginLog方法可以理解成就是一个切入方法，这个方法名是可以程序员自己指定的 showBeginLog
     * 4. JoinPoint joinPoint 在底层执行时，由AspectJ切面框架，会给该切入方法传入 joinPoint对象
     * ，通过该方法程序员可以获取到方法的签名，参数等信息
     *
     * @param joinPoint
     */
    ...
    // 异常通知：即把showExceptionLog方法切入到目标对象方法执行发生异常的catch{}
    ...
    // 最终通知：即把showFinallyEndLog方法切入到目标方法执行后(不管是否发生异常，都要执行，类似于finally{})
    //@After(value = "execution(public float com.hspedu.spring.aop.aspectj.SmartDog.getSum(float,float))")
    //直接使用切入点
    @After(value = "myPointCut()")
    public void showFinallyEndLog(JoinPoint joinPoint) {
        Signature signature = joinPoint.getSignature();
        System.out.println("SmartAnimalAspect-切面类showFinallyEndLog()-方法执行最终执行完毕-日志-方法名-" + signature.getName());
    }
    ...
    //切面表达式也可以指向接口的方法，这时切入表达式会对实现了接口的类/对象生效
    //比如下面我们是对UseInterface 切入，那么对实现类Phone 和 Camera对象都作用了
    @Before(value = "execution(public void com.hspedu.spring.aop.aspectj.UsbInterface.work())")
    public void hi(JoinPoint joinPoint) {
        Signature signature = joinPoint.getSignature();
        System.out.println("SmartAnimalAspect-切面类的hi()-执行的目标方法" + signature.getName());
    }
